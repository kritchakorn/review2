<template>
  <section class="section">
    <table class="table" style="width:100%">
      <thead>
        <tr>
          <th><center><input type="checkbox" id='select_all_invoices' onclick="for(c in document.getElementsByClassName('checkbox')) document.getElementsByClassName('checkbox').item(c).checked = this.checked" >Check</center></th>
          <th>Id</th>
          <th>Plaza Id</th>
          <th>Jboss Status</th>
        </tr>
      ...
      <tbody>
        <tr v-for="(data, index) in lists">
          <td><center>
            <input type="checkbox" class="checkbox" :value="index" v-model="checkes">
...
export default {
  ...
  methods: {
    checkedAll () {
      ...
      var checkinvoices = document.getElementById('checkboxs')
      console.log(blnChecked + '2' + checkinvoices)
      var intLength = checkinvoices.length
      console.log(intLength)
      for (var i = 0; i < intLength; i++) {
        var checinvoice = checkinvoices[i]
        console.log(i)
        checinvoice.checked = true
      }
    },
    btnClick () {
